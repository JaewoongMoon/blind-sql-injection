...
public class DefaultQueries {
	...
	public static final String MSSQL_DB_LENGTH_QUERY = "(select top 1 len(name) from (select top 1 name,dbid from master..sysdatabases order by name asc,dbid desc) as T order by name desc,dbid asc)"; 
