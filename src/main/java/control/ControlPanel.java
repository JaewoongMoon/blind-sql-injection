...
public class ControlPanel extends JPanel {
	...
	public ControlPanel() {
		...
		pauseBtn.setBounds(startBtn.getX() + startBtn.getWidth() + Common.PADDING_X, startBtn.getY(),
				startBtn.getWidth(), startBtn.getHeight());
		stopBtn.setBounds(pauseBtn.getX() + pauseBtn.getWidth() + Common.PADDING_X, startBtn.getY(),
				startBtn.getWidth(), startBtn.getHeight());
	...
	class StartActionHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			// STEP 2. get user input & system config
			UserInput input = inputPanel.getUserInput();
			SystemConfig config = configPanel.getSystemConfig();

			// STEP 3. 로직 처리 요청
			manager.start(input, config);
		}
	}
